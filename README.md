...
A ComfyUI custom node that analyzes an input image using the various AI API platforms and generates a **single, well-structured positive prompt line in a variety of formats** suitable for direct use in image generation workflows.
...
The AI model is selectable per node and passed directly to the Responses API.
